...
public class DangKiDoiRepository {
    ...
    public DangKiDoiRepository(Connection conn) { this.conn = conn; }
    ...
    public DangKiDoi add(DangKiDoi t) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (t.getIdCauLacBo() != null) ps.setInt(3, t.getIdCauLacBo()); else ps.setNull(3, Types.INTEGER);
            ps.setString(4, t.getTenTeam());
            ps.executeUpdate();
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) t.setIdTeam(rs.getInt(1));
            }
            return t;
    ...
    public List<DangKiDoi> findAllBy(int idGiai, int idNoiDung) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, idGiai); ps.setInt(2, idNoiDung);
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
